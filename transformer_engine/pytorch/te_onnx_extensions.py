...
def onnx_cast_to_fp8(g, input, scale, amax, scale_inv, fp8_tensor, otype):
    ...
    return g.op(make_op_name("TRT_FP8QuantizeLinear"), input, scale_inv).setType(
...
def onnx_cast_from_fp8(g, input, scale_inv, fp8_tensor, itype, otype):
    ...
    output_shape = torch.onnx.symbolic_helper._get_tensor_sizes(input)
    out = g.op(make_op_name("TRT_FP8DequantizeLinear"), input, scale_inv).setType(
        input.type().with_dtype(torch.float32).with_sizes(output_shape))
